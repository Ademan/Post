...
/**
 * TransactionReader class reads from file fname, parses customer name, a list
 * of items with quantity, and payment type, then Returns a Transaction object
...
public class TransactionReader {
    ...
    /**
     * Gets the next transaction from file. !!! ASSUMES YOU HAVE CHECKED
     * hasValidTransaction() !!! Reads Customer from the text file, then reads
     * the list of item/quantity pairs,then reads the paymentType/paymentNumber
     * pair. Takes all of this and returns a Transaction object.
     *
     * @return
     */
    ...
    /**
     * Takes a String containing an item/Quantity pair and parses it into a
     * ItemLine object, which it returns
     *
     * @param itemString
     * @return ItemLine object with parsed information
     */
    ItemLine addItem(String itemString) {
        ItemLine newItemLine = null;
        try {
            String itemSplit[] = itemString.split("\\s{2,}");
            if (itemSplit.length == 2) {
                newItemLine = new ItemLine(itemSplit[0], Integer.parseInt(itemSplit[1]));
            } else {
                newItemLine = new ItemLine(itemSplit[0], 1);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    /**
     * Takes a String with paymentInfo/PaymentNumber pair and parses it into a
     * Payment Object, which it returns
     *
     * @param paymentString
     * @return
     */
